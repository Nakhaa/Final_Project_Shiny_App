...
render_homeTab_ui <- function(id, type){
  ...
  tagList(
    sidebarPanel(class="sidebarPanel", width=2,
      ...
      if(type == "one"){
        actionButton(inputId =  "calculate1", label = "Calculate!")
      } else if(type=="two") {
        actionButton(inputId = "calculate2", label = "Calculate!")
      }
    ),
...
renderDistributionPlot <- function(quantile, prob, lower_cv, upper_cv, testStatistics, plotTitle) {
  ggplot(data = data.frame(quantile, prob), aes(x = quantile, y = prob)) +
    geom_line(color = "#4D89B0", size = 1.5) +  # Garis utama dengan warna biru pastel
    ...
    geom_area(data = subset(data.frame(quantile, prob), !is.na(lower_cv) & quantile < lower_cv), 
              aes(x = quantile, y = prob), fill = "#F9A8B1", alpha = 0.4) +  # Gradasi merah muda muda
    ...
    annotate("text", x = testStatistics, y = max(prob)*0.7, 
             ...
             color = "#1A3A5B", fontface = "bold") +  # Bold untuk TS
    
    # Label plot dan tema dengan desain yang lebih elegan
    labs(title = plotTitle,
         x = "Test Statistic",
         y = "Probability Density") +
    theme_minimal(base_size = 14) +  # Tema minimalis dengan ukuran font lebih besar
    theme(
      panel.grid.major = element_line(color = "gray90", size = 0.5),  # Garis grid yang lebih ringan
...
ui <- fluidPage(
  ...
  tags$head(
    ...
    @keyframes slideInFromTop {
      0% {
        transform: translateY(-100%); /* Mulai dari atas (tersembunyi) */
    ...
    @keyframes slideInFromBottom {
      ...
      body{
        ...
        padding: 0px;
        font-family:'Verdana';
        background-image: url('D:/R Project Akhir/Mempercantik_UI/WhatsApp Image 2022-10-17 at 11.06.16.jpg'); 
      ...
      .card-parent {
        animation: slideInFromBottom 0.8s ease-out forwards; /* Apply animasi dengan durasi 0.8 detik */
      ...
      .card-header-child {
        ...
        padding: 10px;
        border-radius: 8px;
        animation: slideInFromTop 0.8s ease-out forwards; /* Apply animasi dengan durasi 0.8 detik */
      }
  ...
  tags$script(HTML("
    MathJax.Hub.Config({
      displayAlign: 'left'  // Mengatur alignment ke kiri
    });
  ...
  div(class = "header_row",
      ...
      div(class= "github_tab",
          actionButton("github_link", icon("github", class="github_icon"), onclick="window.open('https://stackoverflow.com/questions/37795760/r-shiny-add-weblink-to-actionbutton')")
      ),
  ),
